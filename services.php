...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!verify_csrf_token($_POST['csrf'] ?? '')) {
    redirect_with_message('/services.php', 'Invalid CSRF token.', 'error');
  }
  ...
  if ($serviceId <= 0 || $name === '') {
    redirect_with_message('/services.php', 'Please fill required fields correctly.', 'error');
  }
  $stmt = $db->prepare('INSERT INTO service_registrations (service_id, full_name, email, phone, notes) VALUES (?, ?, NULL, ?, ?)');
  $stmt->execute([$serviceId, $name, $phone !== '' ? $phone : null, $notes !== '' ? $notes : null]);
  redirect_with_message('/services.php?sid=' . $serviceId, 'Registered successfully. We will contact you.', 'success');
}

$services = $db->query('SELECT id, title, requirements, short_description FROM services ORDER BY id DESC')->fetchAll();
// $flash already set earlier; do not clear again
...
<head>
  ...
  <title>Services - ISKCON Portal</title>
  ...
  <style>
    ...
    * {
    ...
    .register-btn:hover {
      transform: translateY(-3px);
      box-shadow: 0 10px 15px rgba(106, 48, 147, 0.3);
    }
    ...
    .register-toggle:checked + .register-btn::after {
      content: '\f068';
    }
    ...
    .form-container {
      ...
      width: 100%;
         ...
         .register-toggle:checked ~ .form-container {
       max-height: 1000px;
       margin-top: 1.5rem;
     }
<?php if (isset($_GET['sid'])): ?>
     .card[data-sid="<?php echo (int)$_GET['sid']; ?>"] .form-container {
       max-height: 1000px;
       margin-top: 1.5rem;
     }
<?php endif; ?>
    ...
    .submit-btn {
      ...
      justify-content: center;
      gap: 0.8rem;
      box-shadow: 0 4px 6px rgba(247, 183, 51, 0.2);
    }

    .submit-btn:hover {
      background: linear-gradient(135deg, var(--accent-dark), var(--accent-color));
      transform: translateY(-3px);
      box-shadow: 0 10px 15px rgba(247, 183, 51, 0.3);
    }
    ...
    @keyframes fadeInDown {
      ...
      to { opacity: 1; transform: translateY(0); }
          ...
          .alert.success, .card[data-sid="<?php echo isset($_GET['sid']) ? (int)$_GET['sid'] : 0; ?>"] .status-success {
        background-color: #f0fff4;
        color: #2f855a;
        border-left: 4px solid #48bb78;
        outline: 2px solid rgba(72, 187, 120, 0.25);
        outline-offset: -2px;
      }
...
<div class="main-content">
  <div class="container">
    ...
    <?php $flash = $flash ?? get_and_clear_flash(); ?>
    <?php if ($flash): ?>
      <div id="status" class="alert <?php echo $flash['type']; ?>" role="status" aria-live="polite">
        <span><?php echo htmlspecialchars($flash['message']); ?></span>
        <button class="alert-close" onclick="this.parentElement.remove()" aria-label="Close message">&times;</button>
      </div>
    <?php endif; ?>
    ...
    <div class="grid">
      <?php if (empty($services)): ?>
        <div class="empty-message">
          ...
          <p>Please check back later for upcoming spiritual services.</p>
        </div>
      <?php else: ?>
                  <?php foreach ($services as $sv): ?>
            <div class="card" data-sid="<?php echo (int)$sv['id']; ?>">
            <h3><?php echo htmlspecialchars($sv['title']); ?></h3>
            <p><?php echo nl2br(htmlspecialchars($sv['short_description'])); ?></p>
            <div class="requirements">
              <p><strong>Requirements:</strong> <?php echo nl2br(htmlspecialchars($sv['requirements'])); ?></p>
            </div>

            <input type="checkbox" id="register-<?php echo (int)$sv['id']; ?>" class="register-toggle">
            <label for="register-<?php echo (int)$sv['id']; ?>" class="register-btn">
              Register Now
            </label>
            
                         <div class="form-container">
               <?php if ((int)($_GET['sid'] ?? 0) === (int)$sv['id']): ?>
                  <div class="alert success status-success" role="status">Registration successful.</div>
                <?php endif; ?>
               <form method="post">
                ...
                <div class="form-row">
                  ...
                  <textarea name="notes" placeholder="Any special requests or information"></textarea>
                </div>
...
<script>
  // Auto-hide success message after 6 seconds, keep errors visible
  document.addEventListener('DOMContentLoaded', function() {
    const status = document.getElementById('status');
    if (status) {
      // Scroll into view on load for visibility
    ...
    // Also scroll the submitted card into view if sid is present
    const submittedCard = document.querySelector('.card[data-sid="'+ new URLSearchParams(location.search).get('sid') +'"]');
    if (submittedCard) {
